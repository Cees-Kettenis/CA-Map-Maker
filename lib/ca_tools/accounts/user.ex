defmodule CATools.Accounts.User do
  use Ecto.Schema
  ...
  defp validate_email(changeset, opts) do
    ...
    if Keyword.get(opts, :validate_unique, true) do
      changeset
      |> unsafe_validate_unique(:email, CATools.Repo)
  ...
  """
  @spec valid_password?(t() | term(), term()) :: boolean()
  def valid_password?(user, password) do
    case {user, password} do
      {%CATools.Accounts.User{hashed_password: hashed_password}, password}
      when is_binary(hashed_password) and is_binary(password) and byte_size(password) > 0 ->
